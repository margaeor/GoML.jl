...
function add_data!(bbr::BlackBoxRegressor, X::DataFrame, Y::Array)
    ...
    # throw(GoMLException("A majority of samples on BBR $(bbr.name) evaluated as infeasible. " * 
...
function evaluate(bbl::BlackBoxLearner, data::Union{Dict, DataFrame})
    ...
    if isnothing(bbl.f)
        ...
        if any(isinf.(vals)) || any(isnan.(vals))
            throw(GoMLException(string("Constraint ", bbl.constraint, " returned an infinite or NaN value.",
...
function active_lower_tree(bbr::BlackBoxRegressor)
    if length(bbr.active_trees) > 2
        throw(GoMLException("Regressor $(bbr.name) has too many active trees. There is a bug in the update. "))
