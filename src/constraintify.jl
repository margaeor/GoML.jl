...
function add_tree_constraints!(gm::GlobalModel, bbc::BlackBoxClassifier, idx = length(bbc.learners))
    isempty(bbc.mi_constraints) || throw(GoMLException("BBC $(bbc.name) already has associated MI approximation."))
    isempty(bbc.leaf_variables) || throw(GoMLException("BBC $(bbc.name) already has associated MI variables."))
    ...
    elseif size(bbc.X, 1) == 0
        throw(GoMLException("Constraint " * string(bbc.name) * " has not been sampled yet, and is thus untrained."))
    elseif isempty(bbc.learners)
        throw(GoMLException("Constraint " * string(bbc.name) * " must be learned before tree constraints
                            can be generated."))
    elseif bbc.feas_ratio == 0.0
        throw(GoMLException("Constraint " * string(bbc.name) * " is INFEASIBLE but you tried to include it in
    ...
    elseif !get_param(gm, :ignore_accuracy) && !check_accuracy(bbc)
        throw(GoMLException("Constraint " * string(bbc.name) * " is inaccurately approximated. "))
    else
        lnr = bbc.learners[idx]
        if lnr isa AbstractClassifier
            bbc.mi_constraints, bbc.leaf_variables = embed_mio!(lnr, gm, bbc)
...
function add_tree_constraints!(gm::GlobalModel, bbr::BlackBoxRegressor, idx = length(bbr.learners))
    ...
    if size(bbr.X, 1) == 0 && !get_param(bbr, :reloaded)
        throw(GoMLException("Constraint " * string(bbr.name) * " has not been sampled yet, and is thus untrained."))
    ...
    elseif isempty(bbr.learners)
        throw(GoMLException("Constraint " * string(bbr.name) * " must be learned before tree constraints
                            can be generated."))
    elseif isempty(bbr.ul_data) && !(bbr.learners[idx] isa AbstractRegressor)
        throw(GoMLException("Constraint " * string(bbr.name) * " is a Regressor, 
    ...
    if !isempty(bbr.lls)
        bbr.thresholds[idx].first == "reg" || 
            throw(GoMLException("BBRs with LinkedRegressors are only allowed ORT approximators."))
        length(bbr.active_trees) == 0 || 
            throw(GoMLException("BBRs with LinkedRegressors cannot have more than one active tree."))
    elseif bbr.thresholds[idx].first == "reg"
        (isempty(bbr.leaf_variables) || !isnothing(bbr.thresholds[idx].second)) ||
            throw(GoMLException("Please clear previous tree constraints from $(gm.name) " *
        ...
        if !isnothing(bbr.thresholds[idx].second) && !isempty(bbr.active_trees)
            bbr.thresholds[active_upper_tree(bbr)].second == bbr.thresholds[idx].second || 
                throw(GoMLException("Upper-thresholded regressors must be preceeded by upper classifiers " * 
    ...
    elseif bbr.thresholds[idx].first == "rfreg"
        isempty(bbr.leaf_variables) ||
            ...
            !isempty(bbr.lls) && throw(GoMLException("Random Forests cannot be used to approximate " *
                        "linked BBR $(bbr.name)."))
        isnothing(bbr.thresholds[idx].second) ||
            throw(GoMLException("RandomForestRegressors are not allowed upper bounds."))
        bbr.equality && 
            throw(GoMLException("RandomForest cannot approximate BBR $(bbr.name) since it is an equality constraint."))
    elseif bbr.thresholds[idx].first == "upper"
        all(collect(keys(bbr.mi_constraints)) .>= 0)  || throw(GoMLException("Please clear previous upper tree constraints from $(gm.name) " *
                                                          "before adding new constraints."))
    elseif bbr.thresholds[idx].first == "lower"
        all(collect(keys(bbr.mi_constraints)) .<= 0) || throw(GoMLException("Please clear previous lower tree constraints from $(gm.name) " *
                                                            "before adding new constraints."))
    end
...
function add_regr_constraints!(m::JuMP.Model, x::Array{JuMP.VariableRef}, y::JuMP.VariableRef, 
    ...
    elseif lnr isa OptimalTreeClassifier
        isempty(lrs) || throw(GoMLException("Bug: Cannot use OCTs to approximate linked BBRs."))
        # Add a binary variable for each leaf
        all_leaves = find_leaves(lnr)
        # Add a binary variable for each leaf
        ...
        if leaf_key <= 0
            while !isempty(leaf_constrs)
                ...
                if is_valid(gm.model, constr)
                    ...
                    throw(GoMLException("Bug: Constraints could not be removed."))
                end
            end
        ...
        if leaf_key <= 0
            for leaf_var in leaf_vars
                if is_valid(gm.model, leaf_var)
                    ...
                    throw(GoMLException("Bug: Variables could not be removed."))
                end
            end
            if is_valid(gm.model, bin_var)
                delete(gm.model, bin_var)
            else
                throw(GoMLException("Bug: Binary variable could not be removed."))
            end
            if is_valid(gm.model, aux_dep)
                delete(gm.model, aux_dep)
            else
                throw(GoMLException("Bug: Aux dependent variable could not be removed."))
            end
...
function update_tree_constraints!(gm::GlobalModel, bbr::BlackBoxRegressor, idx = length(bbr.learners))
    ...
    elseif length(bbr.active_trees) == 2 # two sets of mi_constraints
        ...
        elseif hypertype in valid_uppers
            ...
            throw(GoMLException("Hypertype $(hypertype) not recognized."))
        end
        add_tree_constraints!(gm, bbr, idx)
        return 
    else
        throw(GoMLException("$(gm.name) has too many active lower/upper bounding regressors."))
    end
end
