...
function check_sampled(bbl::BlackBoxLearner)
    size(bbl.X, 1) == 0 && throw(GoMLException(string("BlackBoxLearner ", bbl.name, " must be sampled first.")))
    return 
end
...
function learn_constraint_IAI!(lnr::Union{IAI.OptimalTreeRegressor, IAI.OptimalTreeClassifier, 
    ...
    if check_feasibility(bbc) || get_param(bbc, :ignore_feasibility)
        ...
        throw(GoMLException("Not enough feasible samples for BlackBoxClassifier " * string(bbc.name) * "."))
    end
end
...
function learn_constraint!(bbc::BlackBoxClassifier, algs::Union{Nothing, Array{String}} = nothing; kwargs...)
    ...
    for alg in algs
        ...
        if !haskey(LEARNER_DICT["classification"], alg)
            throw(GoMLException("$(alg) model is not supported for classification."))
        end
    ...
    for alg in algs 
        ...
        try
            ...
            elseif threshold.first == "rfreg"
                ...
                bbr.local_convexity < 0.75 || throw(GoMLException("Cannot use RandomForestRegressor " *
                ...
                throw(GoMLException("$(threshold.first) is not a valid learner type for" *
