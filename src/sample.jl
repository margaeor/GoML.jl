...
function knn_sample(bbl::BlackBoxClassifier; k::Int64 = 10, sample_density = 1e-5, sample_idxs = nothing, sign = 1)
    if bbl.feas_ratio == 0. || bbl.feas_ratio == 1.0
        throw(GoMLException("Constraint " * string(bbl.name) * " must have at least one feasible or
...
function uniform_sample_and_eval!(bbl::BlackBoxLearner; sampling_methods::Array{String}=["boundary", "lh", "knn", "derivative", "oct"],
    ...
    if bbl isa BlackBoxClassifier
        ...
        elseif bbl.feas_ratio == 0.0
            throw(GoMLException(string(bbl.name) * " has zero feasible samples. " *
...
function last_leaf_sample(bbc::BlackBoxClassifier, n_samples = get_param(bbc, :n_samples))
    isempty(bbc.active_leaves) &&         
        throw(GoMLException("BBC $(bbc.name) needs to be optimized first, to figure out its active leaves."))
    if !bbc.equality
...
function last_leaf_sample(bbr::BlackBoxRegressor, n_samples = get_param(bbr, :n_samples))
    if isempty(bbr.active_leaves)
        throw(GoMLException("BBR $(bbr.name) needs to be optimized first, to figure out its active leaves."))
    ...
    elseif length(bbr.active_trees) == 1
        ...
        throw(GoMLException("No active trees found in BBR $(bbr.name) while attempting to leaf-sample."))
    end
end
...
function opt_sample(gm::GlobalModel, bbl::BlackBoxLearner)
    ...
    for i in 1:size(Xc, 1)
        ...
        x0 = DataFrame(Dict(string(p[1]) => rand(Uniform(p[2][1], p[2][2])) for p in GoML.get_bounds(gm)))
        for col in names(x0)
...
#         elseif length(obj_bbl) > 1
#             throw(GoMLException("GlobaModel $(gm.name) has more than one BlackBoxRegressor" *
...
function oct_sampling(bbl::BlackBoxClassifier, sampling_factor=0.5)
    ...
    try
        ...
        #     try
                ...
                lnr = GoML.LEARNER_DICT["classification"]["OCT"]()
                sub_idx = StatsBase.sample(all_idx, trunc(Int, size(bbl.X,1)*0.8))
                lnr, score = GoML.learn_from_data!(copy(bbl.X[sub_idx,:]), 1.0*(copy(bbl.Y[sub_idx]) .>= 0), lnr, nothing; use_test_set=false)
                
                # IAI.write_json("dump/oct_exports/lnr_$(bbl.name)_$(i).json", lnr)

                upper_dict, lower_dict = GoML.trust_region_data(lnr, Symbol.(bbl.expr_vars))
