...
function create_noncvx_qp_models(n_vars, n_constr, algs, linear_objective=true, relax_coeff=0; seed=123)
    ...
    local function create_gm_model(obj_expr, constr_expr)
        ...
        lbs_dict = Dict(x .=> lbs)
        ubs_dict = Dict(x .=> ubs)
        GoML.bound!(gm, Dict(var => [lbs_dict[var], ubs_dict[var]] for var in gm.vars if var != relax_var))
        
        # Add objective
        # @TODO: see why expr_vars argument is needed
        GoML.add_nonlinear_or_compatible(gm, 
            obj_expr, vars=x[1:N], 
        ...
        for constr in constr_expr
            GoML.add_nonlinear_constraint(gm, constr, vars=x[1:N], expr_vars=[x[1:N]], alg_list=algs, equality=EQUALITIES, regression=false)
            #constr_hash = bytes2hex(sha1("$(repr(constr))$(EQUALITIES)"))
            #push!(hashes, constr_hash)
        end
        
        GoML.set_param(gm, :ignore_accuracy, true)
        #GoML.set_param(gm, :ignore_feasibility, true)
...
function solve_baron(gb, gm_hash, N)
    ...
    baron_output_path = "dump/qp/baron/baron_new.csv"
...
function solve_and_benchmark(folders; alg_list = ["GBM", "SVM"])
    ...
    for N = [5, 10, 20, 30, 40, 50, 60, 70]
        for M = [1,5,10,15]
            ...
            gm_hash = GoML.calculate_hash(gm)
