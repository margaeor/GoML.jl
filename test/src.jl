...
- Author: Berk
- Date: 2020-06-16
This tests everything to do with the core of the GoML code,
...
function test_expressions()
    ...
    @test GoML.get_var_ranges(expr_vars) == [(1:5),(6:8),9]
    @test GoML.zeroarray([(1:5),(6:8),9]) == [zeros(5), zeros(3), 0]
    flat_expr = :((x...) -> $(expr)([x[i] for i in $(GoML.get_var_ranges(expr_vars))]...))
    ...
    @test GoML.get_varmap(vars, flatvars) == [(2,2), (3,0), (1,1), (1, 2), (1,3), (1,4)]
    @test GoML.get_datamap(vars, flatvars) == [7, 9, 1, 2, 3, 4]
    ...
    @test_throws GoMLException gradientify(@constraint(model, [x[1] x[2];
...
function test_bounds()
    ...
    new_bound = x[4] => [-10,-6]
    @test_throws GoMLException GoML.check_infeasible_bound(new_bound)
    @test_throws GoMLException bound!(model, new_bound)
...
function test_bbc()
    ...
    @test_throws GoMLException X = lh_sample(bbl, n_samples=100);

    # Check knn_sampling without previous samples
    @test_throws GoMLException knn_sample(bbl)
...
function test_bbr()
    ...
    @test_throws GoMLException last_leaf_sample(bbcs[1])
...
function test_basic_gm()
    ...
    @test_throws GoMLException surveysolve(gm) # no data error
    uniform_sample_and_eval!(gm)
    surveysolve(gm)
